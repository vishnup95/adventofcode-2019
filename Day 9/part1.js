console.time("Operation Ceres Start");
...
const firstInput = 1; // change to 1 for part 1.

performOpCodeOperation(opCodes, firstInput);
...
function performOpCodeOperation(codes, input) {
  ...
  while (index < codes.length && codes[index] != 99) {
    ...
    let paddedOpCode = stringOpCode.padStart(5, "0");
    let finalOpCode = paddedOpCode[3].concat(paddedOpCode[4]);

    let value1 = calculateValue(paddedOpCode[2], codes, index, relativeBase, 1);
    let value2 = calculateValue(paddedOpCode[1], codes, index, relativeBase, 2);
    let value3 = calculateIndex(paddedOpCode[0], codes, index, relativeBase);
    ...
    switch (finalOpCode) {
      ...
      case "03":
        ...
        } else if (paddedOpCode[2] === "1") {
          codes[index + 1] = input;
        } else {
          codes[relativeBase + codes[index + 1]] = input;
        }
        index += 2;
        break;
...
function calculateValue(paramMode, codes, index, relativeBase, n) {
  if (paramMode === "0") {
    if (codes[codes[index + n]] === undefined) {
      codes[codes[index + n]] = 0;
    }
    return codes[codes[index + n]];
  } else if (paramMode === "1") {
    return codes[index + n];
  } else {
    if (codes[relativeBase + codes[index + n]] === undefined) {
      codes[relativeBase + codes[index + n]] = 0;
    }
    return codes[relativeBase + codes[index + n]];
  }
}

function calculateIndex(paramMode, codes, index, relativeBase) {
  if (paramMode === "0") {
    return codes[index + 3];
  } else {
    return relativeBase + codes[index + 3];
  }
}

console.timeEnd("Operation Ceres Start")
